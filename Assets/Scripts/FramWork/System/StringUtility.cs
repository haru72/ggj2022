...
{
	static public string StringToByteAryStr( string str )
	{
		return EncryptString( str );

		/*
		string ret = "";
		var strByteAry = System.Text.Encoding.UTF8.GetBytes( str );
		foreach( var strByte in strByteAry )
		{
			if( ret != "" )
			{
				ret += "#";
	...
	static public string ByteAryStrToString( string byteAryStr )
	{
		return DecryptString( byteAryStr );
		/*	List<byte> byteList = new List<byte>();
			var ary = byteAryStr.Split( '#' );
			foreach( var str in ary )
			{
				byteList.Add( byte.Parse( str ) );
			}
